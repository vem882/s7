...
public Action:Event_End(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(client) && CanView[client])
	{
		...
		if (HTTPCommSupported && GetConVarInt(g_Preload) > 0)
		{
			...
			CreateTimer(0.1, PreloadMotdTimer, client);
			
			CreateTimer(3.0, PreloadCheckTimer, client);
		}
		else
		{
		...
		CreateTimer((GetConVarFloat(g_Review) * 60), ReviewMotdTimer, client);
	}
...
public Action:Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CreateTimer(0.5, CheckPlayerDeath, client);
	
	return Plugin_Continue;
}

public Action:CheckPlayerDeath(Handle:timer, any:client)
{
...
public Action:OnVGUIMenu(UserMsg:msg_id, Handle:bf, const players[], playersNum, bool:reliable, bool:init)
{
	...
	if (HTTPCommSupported && GetConVarInt(g_Preload) > 0)
	{
		...
		CreateTimer(2.0, PreloadCheckTimer, client);
	}
	else
	{
		if (!StrEqual(gameDir, "left4dead2") && !StrEqual(gameDir, "left4dead"))
...
public Action:PreloadMotdTimer(Handle:timer, any:client)
{
	if (!IsValidClient(client))
		return;
...
public Action:PreloadCheckTimer(Handle:timer, any:client)
{
	if (!IsValidClient(client))
		return;
